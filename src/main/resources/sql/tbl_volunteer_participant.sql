...
CREATE TABLE TBL_VOLUNTEER_PARTICIPANT (
                                 ...
                                 CONSTRAINT FK_PARTICIPANT_ADMIN FOREIGN KEY(ADMIN_ID)
                                     REFERENCES TBL_ADMIN(ID) ON DELETE CASCADE ,
                                 CONSTRAINT FK_PARTICIPANT_COURSE FOREIGN KEY(COURSE_ID)
                                     REFERENCES TBL_COURSE(ID) ON DELETE CASCADE
);
COMMENT ON TABLE TBLV_PARTICIPANT IS '봉사코스의 참여자 정보를 저장하는 테이블';
COMMENT ON COLUMN TBL_VOLUNTEER_PARTICIPANT.ID IS 'TBL_PARTICIPANT의 고유 식별자, 기본 키';
COMMENT ON COLUMN TBL_VOLUNTEER_PARTICIPANT.ADMIN_ID IS '참여자의 회원 ID';
...
COMMENT ON COLUMN TB_VOLUNTEERL_PARTICIPANT.UPDATED_DATE IS '데이터 수정 날짜, 기본값은 SYSDATE';

SELECT *FROM TBL_PARTICIPANT;
SELECT * FROM TBL_PLAN ORDER BY ID DESC ;
