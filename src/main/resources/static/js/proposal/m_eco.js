    ...
    if (window.innerWidth > 600) {
        ...
        swiperslides.forEach((swiperslide) => {
            swiperslide.addEventListener("click", (e) => {
                // console.log("pc  " + swiperslidesLength);
                // console.log("pc  " + e.target.closest("li").outerHTML);
                // console.log("pc  " + e.target.closest("li").querySelector("em").outerHTML);
        ...
        document.querySelector(".swiper-button-next").addEventListener("click", (e) => {
            ...
            if (rightEnd > maxRightEnd) {
                ...
                console.log("leftEnd " + leftEnd + "rightEnd " + rightEnd + "maxRightEnd " + maxRightEnd);
        ...
        let maxReviewRightEnd = document.querySelectorAll("#web .slick-slide").length * 244;
        ...
        // 코스 지도 보여주기
        var mapContainer = document.querySelector("#map1"), // 지도를 표시할 div
            mapOption = {
                center: new kakao.maps.LatLng(35.409476, 127.396059), // 지도의 중심좌표
                level: 9, // 지도의 확대 레벨
            };
        ...
        geocoder.addressSearch("올림픽로 19-2", (result, status) => {
            if (status === kakao.maps.services.Status.OK) {
                ...
                let mapContainer = document.querySelector("#map2"), // 지도를 표시할 div
                    mapOption = {
                        center: coords,
                        level: 3, // 지도의 확대 레벨
                    };
        ...
        // 화면이 412 보다 작을 때 mobile 모바일

        // 화면이 눌릴 때 위에 빨간줄 이동하고, 아래쪽 관광지 상세 사진 보이기
        const cosList = document.querySelector("#mobile .pc.js_slider .cosList");
        const swiperslides = cosList.querySelectorAll(".swiper-slide");
        const swiperslidesLength = swiperslides.length;

        swiperslides.forEach((swiperslide) => {
            swiperslide.addEventListener("click", (e) => {
                // console.log("mobile  " + swiperslidesLength);
                // console.log("mobile  " + e.target.closest("li").outerHTML);
                // console.log("mobile  " + e.target.closest("li").querySelector("em").outerHTML);
                const emValue = parseInt(e.target.closest("li").querySelector("em").textContent);
                ...
                const idValue = emValue < 10 ? "cosTab0" + emValue : "cosTab" + emValue;
                document.querySelectorAll("#mobile .cos_cont").forEach((e) => {
                    //console.log(e.className);
                    e.classList.remove("active");
                    if (e.id == idValue) {
                        e.classList.add("active");
        ...
        // 연관 관광지 버튼 클릭시 이미지 1개씩 이동 200px, 디스플레이 362px
        let leftEnd = 0; // 화면 왼쪽 끝
        let rightEnd = 400; // 화면에 실제 보여지는건 1개 이상이므로 2개로
        let maxRightEnd = swiperslidesLength * 200;

        document.querySelector("#mobile .swiper-button-next").addEventListener("click", (e) => {
            rightEnd += 200;
            leftEnd += 200;
            if (rightEnd > maxRightEnd) {
                // maxRightEnd 보다 오른쪽으로 못 가도록 막음
                leftEnd -= rightEnd - maxRightEnd;
                rightEnd = maxRightEnd;
                document.querySelector("#mobile .swiper-button-next").classList.add("swiper-button-disabled");
                document.querySelector("#mobile .swiper-button-next").setAttribute("aria-disabled", "true");
            }
            cosList.style.transform = `translate3d(${362 - rightEnd}px, 0, 0)`;
            document.querySelector("#mobile .swiper-button-prev").classList.remove("swiper-button-disabled");
            document.querySelector("#mobile .swiper-button-prev").setAttribute("aria-disabled", "false");
        });

        document.querySelector("#mobile .swiper-button-prev").addEventListener("click", (e) => {
            leftEnd -= 200;
            rightEnd -= 200;
            if (leftEnd < 0 || leftEnd == 0) {
                ...
                document.querySelector("#mobile .swiper-button-prev").classList.add("swiper-button-disabled");
                document.querySelector("#mobile .swiper-button-prev").setAttribute("aria-disabled", "true");
            }
            cosList.style.transform = `translate3d(${-leftEnd}px, 0, 0)`;
            document.querySelector("#mobile .swiper-button-next").classList.remove("swiper-button-disabled");
            document.querySelector("#mobile .swiper-button-next").setAttribute("aria-disabled", "flase");
        });
        // 연관 관광지  버튼 클릭시 작동 1개씩 이동 200px, 디스플레이 362px

        // 후기의 버튼 클릭시 이미지 1개식 이동 244px, 디스플레이 402px

        let leftReviewEnd = 0; // 화면 왼쪽 끝
        let rightReviewEnd = 488; // 화면에 실제 보여지는건 1개 이상이므로 2개로
        let maxReviewRightEnd = document.querySelectorAll("#mobile .slick-slide").length * 244;

        document.querySelector("#mobile .slick-next").addEventListener("click", (e) => {
            rightReviewEnd += 244;
            leftReviewEnd += 244;
            if (rightReviewEnd > maxReviewRightEnd) {
                // maxRightEnd 보다 오른쪽으로 못 가도록 막음
                ...
                document.querySelector("#mobile .slick-next").classList.add("slick-disabled");
            }

            console.log(leftReviewEnd + " " + rightReviewEnd + " " + maxReviewRightEnd);
            document.querySelector("#mobile .slick-track").style.transform = `translate3d(${
                402 - rightReviewEnd
            }px, 0, 0)`;
            document.querySelector("#mobile .slick-prev").classList.remove("slick-disabled");
        });

        document.querySelector("#mobile .slick-prev").addEventListener("click", (e) => {
            leftReviewEnd -= 244;
            rightReviewEnd -= 244;
            if (leftReviewEnd < 0) {
                // 0 보다 왼쪽으로 못 가도록 막음
                ...
                document.querySelector("#mobile .slick-prev").classList.add("slick-disabled");
            }
            document.querySelector("#mobile .slick-track").style.transform = `translate3d(${-leftReviewEnd}px, 0, 0)`;
            document.querySelector("#mobile .slick-next").classList.remove("slick-disabled");
        });
        // 후기의 버튼 클릭시 이미지 1개식 이동 220px, 디스플레이 402px
        ...
        // 코스 지도 보여주기
        var mapContainer = document.querySelector("#mobile #map1"), // 지도를 표시할 div
            mapOption = {
                center: new kakao.maps.LatLng(35.409476, 127.396059), // 지도의 중심좌표
                level: 9, // 지도의 확대 레벨
            };
        ...
        // 화면 확장 축소
        document.querySelector("#mobile #fullMap1").addEventListener("click", (e) => {
            if (mapContainer.style.position === "fixed") {
                mapContainer.style.position = "relative";
                ...
                mapContainer.style.height = "30vh";
                mapContainer.style.zIndex = ""; // 맵이 다른 요소 위에 오도록 설정한거 해제
                document.querySelector("#mobile #fullMap1").style.position = "absolute";
                document.querySelector("#mobile #fullMap1").style.top = "8px";
                // 지도의 중심을 새로운 좌표로 설정
                map.relayout();
                map.setCenter(initialCenter);
            } else {
                mapContainer.style.position = "fixed";
                mapContainer.style.top = "116px";
                mapContainer.style.left = "0px";
                mapContainer.style.width = "100%";
                mapContainer.style.height = "100vh";
                mapContainer.style.zIndex = "1000"; // 맵이 다른 요소 위에 오도록 설정
                document.querySelector("#mobile #fullMap1").style.position = "fixed";
                document.querySelector("#mobile #fullMap1").style.top = "120px";
                // 지도의 중심을 새로운 좌표로 설정
                map.relayout();
                map.setCenter(initialCenter);
            }
        ...
        geocoder.addressSearch("올림픽로 19-2", (result, status) => {
            if (status === kakao.maps.services.Status.OK) {
                let coords = new kakao.maps.LatLng(result[0].y, result[0].x);
                let mapContainer = document.querySelector("#mobile #map2"), // 지도를 표시할 div
                    mapOption = {
                        center: coords,
                        level: 3, // 지도의 확대 레벨
                    };
                ...
                // 화면 확장 축소
                document.querySelector("#mobile #fullMap2").addEventListener("click", (e) => {
                    if (mapContainer.style.position === "fixed") {
                        mapContainer.style.position = "relative";
                        mapContainer.style.top = "0px";
                        mapContainer.style.left = "0px";
                        ...
                        mapContainer.style.height = "30vh";
                        mapContainer.style.zIndex = "";
                        document.querySelector("#mobile #fullMap2").style.position = "absolute";
                        document.querySelector("#mobile #fullMap2").style.top = "8px";
                    } else {
                        mapContainer.style.position = "fixed";
                        mapContainer.style.top = "116px";
                        mapContainer.style.left = "0px";
                        ...
                        mapContainer.style.height = "100vh";
                        mapContainer.style.zIndex = "1000"; // 맵이 다른 요소 위에 오도록 설정
                        document.querySelector("#mobile #fullMap2").style.position = "fixed";
                        document.querySelector("#mobile #fullMap2").style.top = "120px";
                    }
                    map.relayout();
                    // 지도의 중심을 결과값으로 받은 위치로 이동시킵니다
                    map.setCenter(coords);
        ...
        // 위로 버튼 누르면 화면 위쪽으로 천천히 이동
        document.querySelector("#mobile .gQlhwK").addEventListener("click", (e) => {
            window.scrollTo({
                top: 0,
                behavior: "smooth",
            });
        ...
        const JJimMessage = document.createElement("div");
        JJimMessage.className = "Bottom__Wrapper-sc-1nltrn7-0 bpzBFC";
        document.querySelectorAll("#mobile .enp_mobon_cart").forEach((JJim) => {
            JJim.addEventListener("click", () => {
                if (buttonSelected == "true") {
                    document.querySelectorAll("#mobile .enp_mobon_cart img").forEach((img) => {
                        img.src = `data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 32 32' fill='none' xmlns='http://www.w3.org/2000/svg'%3E %3Cg clip-path='url(%23clip0_2517_457)'%3E %3Cpath d='M15.4448 27.0641L15.4368 27.0593L15.4064 27.0433C14.775 26.6998 14.1549 26.3359 13.5472 25.9521C12.0981 25.0404 10.7174 24.0243 9.41595 22.9121C6.47195 20.3729 3.19995 16.5633 3.19995 12.0001C3.20009 10.5112 3.66179 9.05903 4.52147 7.84344C5.38115 6.62785 6.59654 5.70866 8.00027 5.21244C9.40401 4.71622 10.9271 4.66737 12.3597 5.07263C13.7924 5.47788 15.0641 6.31729 16 7.4753C16.9358 6.31729 18.2075 5.47788 19.6402 5.07263C21.0728 4.66737 22.5959 4.71622 23.9996 5.21244C25.4034 5.70866 26.6187 6.62785 27.4784 7.84344C28.3381 9.05903 28.7998 10.5112 28.8 12.0001C28.8 16.5633 25.5296 20.3729 22.584 22.9121C20.7349 24.492 18.7274 25.8764 16.5936 27.0433L16.5632 27.0593L16.5552 27.0641H16.552C16.3819 27.1542 16.1925 27.2014 16.0001 27.2017C15.8078 27.2019 15.6182 27.1553 15.448 27.0657L15.4448 27.0641Z' fill='%23FD2748'/%3E %3C/g%3E %3Cdefs%3E %3CclipPath id='clip0_2517_457'%3E %3Crect width='32' height='32' fill='white'/%3E %3C/clipPath%3E %3C/defs%3E %3C/svg%3E`;
                    });
                    JJimMessage.textContent = "추천 여행을 찜했습니다.";
                    buttonSelected = "false";
                ...
                document.querySelector("#mobile .ifNxJR").appendChild(JJimMessage);

                const bpzBFC = document.querySelector("#mobile .bpzBFC");
                if (!bpzBFC) {
                    // 찜 메세지를 잠시 보여주고 없앰.
                    return;
                }
        ...
        // 로그인 버튼 눌렀을 때 (로그인은 차후 반영)
        document.querySelectorAll("#mobile .ContentComment").forEach((ContentComment) => {
            ContentComment.addEventListener("click", () => {
                if (document.querySelector("#mobile .ContentComment").textContent == "등록") {
                    return;
                }
                document.querySelector("#mobile .replyWrap").className = "replyWrap subscription login";
                document.querySelector("#mobile .writeForm").removeAttribute("style");
                document.querySelectorAll("#mobile .comment").forEach((e) => {
                    e.removeAttribute("readonly");
                    e.removeAttribute("style");
                    e.placeholder = "글을 남겨주세요.";
                });
                document.querySelectorAll("#mobile .fileUp").forEach((e) => {
                    e.removeAttribute("disabled");
                });
                document.querySelectorAll("#mobile .ContentComment").forEach((e) => {
                    e.textContent = "등록";
                });
            });
        });
        ...
        // 댓글에 답글 달기 버튼
        document.querySelectorAll("#mobile .btn2").forEach((btn2) => {
            // 화면에 보여지는 버튼 모으기
            btn2.addEventListener("click", (e) => {
                if (e.target.title == "선택됨") {
                    e.target.setAttribute("title", "");
        ...
        // textarea에 글자 입력시 입력된 글자 수 보여주기
        document.querySelectorAll("#mobile .comment").forEach((comment) => {
            comment.addEventListener("input", (e) => {
                e.target.nextElementSibling.textContent = `${e.target.value.length} / 1200 (추천 글자수: 30자 이내)`;
            });
        });
        ...
        // 댓글 더보기 버튼
        document.querySelector("#mobile .btn_center .btn_more").addEventListener("click", () => {
            const li = document.createElement("li");
            // 향후 데이타를 받을 때는 for문으로 처리
            li.id = "b940dab6-e56b-4103-b120-a1f4c83c5e25";
            li.innerHTML = `
        ...
        <div class="replyBox" style="display: none;">
            ...
            // 댓글 목록 제일 밑에 추가
            document.querySelector("#mobile .list_reply").querySelector("ul").appendChild(li);
            ...
            document.querySelectorAll("#mobile .btn2").forEach((btn2) => {
                if (!btn2.getAttribute("listener")) {
                    ...
                    btn2.addEventListener("click", (e) => {
                        ...
                        // 로그인 됬을 때는 "등록"으로 변경
                        if (document.querySelector("#mobile .replyWrap.login")) {
                            document.querySelectorAll("#mobile .comment").forEach((e) => {
                                e.removeAttribute("readonly");
                                e.removeAttribute("style");
                                e.placeholder = "글을 남겨주세요.";
